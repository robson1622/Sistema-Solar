    ...
    while (window.isOpen())
    {
        ...
        if(i > 500){
            ...
            while(percorrer != NULL){
                ...
                velocidades.setString("Velocidades Angulares (rad/ano terrestre) tomando o\ncentro gravitacional do sistema, em ordem crescente: (aperte Q)");
                velocidades.setPosition(20, 10);
                window.draw(velocidades);
                
                if (this->alinharVelocidadeComAstro(j))
                {
                    nomes.setPosition(20, 30*j + 70);
                    nomes.setCharacterSize(20);
                    velocidades.setString(std::to_string(vetorVelocidades[j]));
                    velocidades.setPosition(130, 30*j + 70);
                }
                    window.draw(velocidades);
                    window.draw(nomes);
